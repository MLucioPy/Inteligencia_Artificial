import os
# 1. SUPRESSÃO DE LOGS E AVISOS DO SISTEMA
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
os.environ['CUDA_VISIBLE_DEVICES'] = '-1' # Força o Streamlit Cloud a usar apenas CPU
...
# CSS para os Cards Industriais e Destaques Estatísticos
st.markdown("""
    <style>
...
# 3. CARREGAMENTO DOS MODELOS SALVOS (VIA CACHE)
@st.cache_resource
def treinar_modelos_industriais():
    # Carrega o dataset preparado para extrair as matrizes de validação do gráfico
    ...
    # Redimensionado para 30 passos para corresponder exatamente ao modelo pré-treinado
    passos_passado = 30
    X_janelas, y_alvos = [], []
    ...
    # Divisão Cronológica/Aleatória alinhada com o ambiente de testes (random_state=42)
    X_train_3D, X_test_3D, y_train, y_test = train_test_split(X_3D, y_3D, test_size=0.2, random_state=42)
    
    X_test_2D = X_test_3D[:, -1, :]
    
    # --- BUSCA E CARREGA O MODELO 1: Random Forest ---
    if os.path.exists('modelo_random_forest.pkl'):
        rf = joblib.load('modelo_random_forest.pkl')
    else:
        st.error("⚠️ Arquivo 'modelo_random_forest.pkl' não encontrado na raiz.")
        st.stop()
        
    y_pred_rf = rf.predict(X_test_2D)
    
    # --- BUSCA E CARREGA O MODELO 2: Rede Neural LSTM ---
    if os.path.exists('modelo_lstm.keras'):
        lstm = load_model('modelo_lstm.keras')
    else:
        st.error("⚠️ Arquivo 'modelo_lstm.keras' não encontrado na raiz.")
        st.stop()
...
with st.spinner("⚙️ Carregando inteligências preditivas e sincronizando dados... Aguarde."):
...
# 5. CÁLCULO DAS PREDIÇÕES EM TEMPO REAL (INFERÊNCIA DIRETA)
...
if "Rede LSTM" in opcao_modelo or "Comparar Ambos" in opcao_modelo:
    ...
    # Ajustado de 6 para 30 passos para bater com o shape do arquivo .keras carregado
    X_sim_3d = np.repeat(array_2d[:, np.newaxis, :], 30, axis=1)
...
st.write("---")
st.caption("Inteligência Artificial - IFSP. Ambiente 100% Otimizado.")
